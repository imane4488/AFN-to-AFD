	...
	//**********************gestion de l'alphabet ***************************//	
		...
		public void ajouterSymbole(){
			...
			if(alphabet.size()!=0){
				...
				System.out.println(s+"a ete ajouté ");}
					}
			else{
				alphabet.add(new Character(s));
			...
			for(int i=0;i<etats.size();i++){
				...
				for(int j=0;j<alphabet.size()+1;j++){
					if(j==alphabet.size()){
						...
						System.out.println("choisissez soit 'vide' ou 'un ou plusieurs etats' (0/1) :  ");
					}else{
						char a=alphabet.get(j);
						System.out.println("donnez le resultat de ("+s+","+a+")");
						System.out.println("choisir : 'vide' ou 'un ou plusieurs etats' (0/1) : ");
					}
						int rep=sc.nextInt();
						if(rep == 0){
